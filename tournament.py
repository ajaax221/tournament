...
def playerStandings():
    ...
    DB = connect()
    c = DB.cursor()
    c.execute("SELECT p.id, p.name, m.win, m.matches FROM players p INNER JOIN matches m ON m.id = p.id order by m.win")
    rows = c.fetchall()
    DB.close()
    return rows
...
def reportMatch(winner, loser):
    ...
    c.execute("INSERT INTO games (idA, idB, win) VALUES (%s,%s,%s)" , (winner,loser,winner,))
    DB.commit()
    DB.close()
